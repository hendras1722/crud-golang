...
func Register(w http.ResponseWriter, r *http.Request) {
	...
	if register.Password != register.PasswordConfirm {
		helpers.Response(w, 400, "Password does not match", nil)
		return
	}
	...
	helpers.Response(w, 201, "User created successfully", user)
}
...
func Login(w http.ResponseWriter, r *http.Request) {
	...
	if err := configs.DB.First(&user, "email = ?", login.Email).Error; err != nil {
		helpers.Response(w, 404, "Wrong email password", nil)
		return
	}
	...
	helpers.Response(w, 200, "User logged in successfully", token)
}
